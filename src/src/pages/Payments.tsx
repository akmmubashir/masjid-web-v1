...
export function Payments() {
  ...
  (selectedHouse?.contributionAmount || 0) * (
  Number(newPayment.monthsCount) || 1) :
  ...
  const handleAddPayment = (e: React.FormEvent) => {
    ...
    const payment: Payment = {
      ...
      type: newPayment.type as any || 'Mahal Charge',
      monthsCount: isMahalCharge ?
      Number(newPayment.monthsCount) || 1 :
      undefined,
      date:
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Track transactions and generate receipts.
          </p>
        </div>
      ...
      <Card>
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by receipt number or house name..."
              className="pl-9"
              value={searchTerm}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="text-masjid-600 dark:text-masjid-400"
                          onClick={() =>
                        ...
                        <button
                          className="p-1 text-slate-400 hover:text-masjid-600 transition-colors"
                          title="View Receipt">
        ...
        <form onSubmit={handleAddPayment} className="space-y-4">
          <Select
            ...
            options={houses.map((h) => ({
              label: `${h.mahalHouseNumber} — ${h.name}`,
              value: h.id
            }))}
              ...
              <div className="rounded-md border border-masjid-100 dark:border-masjid-800 bg-masjid-50/60 dark:bg-masjid-900/20 p-3 flex items-center justify-between">
                <div className="text-sm text-masjid-900 dark:text-masjid-200">
                  ...
                  <span className="text-slate-500 dark:text-slate-400 ml-2 text-xs">
                    ${selectedHouse?.contributionAmount || 0} ×{' '}
                    {newPayment.monthsCount || 1}{' '}
                    {Number(newPayment.monthsCount) === 1 ? 'month' : 'months'}
                  </span>
                </div>
