...
export function MyDashboard() {
  ...
  const outstanding = myPayments.filter((p) => p.status === 'Pending' || p.status === 'Overdue');
  const totalContributed = myPayments.filter((p) => p.status === 'Paid').reduce((s, p) => s + p.amount, 0);
    ...
    return (
      <Card className="max-w-xl mx-auto mt-12">
        <CardContent className="p-8 text-center">
          ...
          <h2 className="font-serif text-xl font-semibold mb-2">{t.myDashboard.noHouseTitle}</h2>
          <p className="text-sm text-slate-500">{t.myDashboard.noHouseDesc}</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">
          {t.myDashboard.greeting}, {user?.name?.split(' ')[0]}
        </h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">
          {t.myDashboard.subtitle} {myHouse.name}.
        </p>
      </div>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="font-serif text-xl font-semibold text-slate-900 dark:text-white">{myHouse.name}</h3>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3 text-sm text-slate-600 dark:text-slate-300">
                  <Badge variant="outline">
                    <Hash className="h-3 w-3 mr-1 inline" />{myHouse.mahalHouseNumber}
                  </Badge>
                  <span className="flex items-center">
                    <UsersIcon className="h-4 w-4 mr-1.5" />{myHouse.members.length} {t.houses.members}
                  </span>
                  <Badge variant={myHouse.status === 'Active' ? 'success' : 'default'}>{myHouse.status}</Badge>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>{t.myDashboard.yourContribution}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid grid-cols-2 gap-4 mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <div>
                ...
                <p className="text-xl font-semibold text-slate-900 dark:text-white">{myPayments.length}</p>
              </div>
            </div>
            <div className="mt-5 p-3 bg-masjid-50 dark:bg-masjid-900/20 rounded-md flex items-start gap-2 text-sm text-masjid-900 dark:text-masjid-200">
              ...
              <span>{t.myDashboard.contributionNote}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t.myDashboard.paymentStatus}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-slate-500">{t.myDashboard.lastPayment}</p>
                <p className="font-medium text-slate-900 dark:text-white">
                  {lastPaid ? `${lastPaid.amount} on ${lastPaid.date}` : t.myDashboard.noPaymentsYet}
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className={`rounded-full p-2 ${outstanding.length > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-slate-100 dark:bg-slate-800'}`}>
                <AlertCircle className={`h-4 w-4 ${outstanding.length > 0 ? 'text-red-600' : 'text-slate-400'}`} />
              </div>
              <div>
                <p className="text-xs text-slate-500">{t.myDashboard.outstanding}</p>
                <p className="font-medium text-slate-900 dark:text-white">
                  {outstanding.length > 0 ? `${outstanding.length} ${t.myDashboard.pending}` : t.myDashboard.allUpToDate}
                </p>
              </div>
            </div>
            <Button variant="outline" className="w-full mt-2">{t.myDashboard.viewFullHistory}</Button>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center">
            ...
            {t.myDashboard.latestAnnouncements}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {recentAnnouncements.map((a) => (
            <div key={a.id} className="border-b border-slate-100 dark:border-slate-800 last:border-0 pb-3 last:pb-0">
